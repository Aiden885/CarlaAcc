...
class acc:
    ...
    def __init__(self, use_result_plotter=None, scenario_mode=None):
        ...
        self._apply_plotter_mode(use_result_plotter)
        ...
        if self.enable_realtime:
            ...
            print(f"⏱️  实时模式已启用（1:1速度，{self.config.realtime_target_fps} FPS）")
        else:
    ...
    def _process_input_event(self, event: dict):
        ...
        if event_type == 'quit':
            ...
            print(f"主开关 ACC master switch: {'ON' if new_state else 'OFF'}")
            ...
            print(f"ACC debug: {'ON' if self.acc_controller.debug else 'OFF'}")
            ...
            print(f"⌨️ 已记录键盘指令: {event['desc']} (将在下一帧处理)")
    ...
    def _update_plotter(self, step_result, manual_input_state):
        ...
        if env_data.get('control_mode_flag') == 1 and env_data.get('has_target'):
            # 计算req_torque和req_decel（简化版）
            control_output_val = unified_output.get('sppvt_control_output', 0.0)
            if env_data.get('control_mode_flag') == 1:
                sppvt_accel = -control_output_val
            else:
                sppvt_accel = control_output_val

            if sppvt_accel >= 0:
                req_torque = sppvt_accel * self.config.sppvt_accel_scale
                req_decel = 0.0
            else:
                req_torque = 0.0
                req_decel = abs(sppvt_accel * self.config.sppvt_decel_scale)
