...
class acc:
    ...
    def init_carla(self):
        ...
        # num_regular_vehicles = min(3, len(spawn_points) - 1)
        # vehicles = []

        # # Spawn 10 regular vehicles
        # for i in range(num_regular_vehicles):
        #     spawn_point = spawn_points[0]
        #     spawn_point.location.x += 10 * i
        #     try:
    ...
    def radar_callback(self, radar_data):
        ...
        for detection in radar_data:
            try:
                ...
                if z > -0.5 and abs(velocity - expected_static_velocity) > velocity_tolerance:
                    self.radar_points.append([x, y, z, vx, vy, vz, velocity])
            except AttributeError as e:
                print(f"AttributeError: {e}. Raw detection: {detection}")
                break
        # print(len(radar_data) ,"   ", len(self.radar_points))
        if self.radar_points:
            fields = [PointField('x', 0, PointField.FLOAT32, 1),
                ...
                PointField('v', 24, PointField.FLOAT32, 1)]
            header = rospy.Header()
