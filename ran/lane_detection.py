...
#         l_thresh_min = 220 #np.mean(l_channel)#200
#         l_thresh_max = 255
#         s_thresh_min = 0
#         s_thresh_max = 150
#
...
#         return 0, 0 #straight
#
#     def sliding_windows(self, binary_warped, nwindows=30, margin=40, minpix=100, maxpix=1000, move_step = 10, move_num = 3):
...
class LaneDetector:
    ...
    def sliding_windows(self, binary_warped, nwindows=60, margin=30, minpix=40, maxpix=600, move_step = 20, move_num = 3):
        # Create an output image to draw on
        out_img = np.dstack((binary_warped, binary_warped, binary_warped)) * 255
        out_img = out_img.astype(np.uint8)  # Explicitly convert to uint8
